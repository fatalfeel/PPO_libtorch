...
Categorical::Categorical(torch::Tensor& probs)
{
    ...
    //m_param = probs != nullptr ? probs : logits;
    m_param 		= probs;
...
torch::Tensor Categorical::Sample(torch::ArrayRef<int64_t> sample_shape)
{
    ...
    auto exp_probs = m_probs.expand(param_shape);
...
torch::Tensor Categorical::Get_Logits()
{
	return m_logits;
}

torch::Tensor Categorical::Get_Probs()
{
	return m_probs;
}
